...
const ImportProducts = ({ token }) => {
  ...
  const [activeTab, setActiveTab] = useState('import'); // 'import', 'files', 'folders', 'batches'
  ...
  const handleDeleteBatch = async (batchId) => {
    if (!window.confirm("Are you sure you want to delete all products from this batch? This action cannot be undone.")) {
      return;
    }
  ...
  // Load image folders on component mount
  useEffect(() => {
    if (token) {
      fetchImageFolders();
    }
    ...
    <main className="p-6 max-w-screen-2xl mx-auto bg-gradient-to-b from-indigo-50 to-white min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 border border-indigo-100">
            ...
            <form onSubmit={handleImport} className="space-y-8">
                ...
                <div className="relative">
                  ...
                  <div className="mt-3 bg-indigo-50 rounded-lg p-3">
                    ...
                    <p className="text-xs text-indigo-600">
                      Any filename is acceptable - category is selected below
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="block text-base font-semibold text-indigo-800">
                  Product Category <span className="text-purple-500">*</span>
                </label>
                <div className="relative">
                  <select
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="block w-full py-3 px-4 border border-indigo-300 bg-white rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    disabled={importing}
                    required
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <div className="mt-3 bg-indigo-50 rounded-lg p-3">
                    <p className="text-xs text-indigo-600">
                      <span className="font-semibold block mb-1 text-indigo-800">Image Naming Guidelines:</span>
                      <ul className="list-disc ml-6 mt-2 text-sm">
                        <li className="mb-1">For a single image: <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U.jpg</span> (same as Model No.)</li>
                        <li className="mb-1">For multiple images (up to 4): Use one of these naming patterns:
                          <div className="flex flex-wrap gap-2 mt-1 mb-2">
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U.jpg</span> (primary)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_1.jpg</span> (2nd view)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_2.jpg</span> (3rd view)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_3.jpg</span> (4th view)
                          </div>
                          <div className="text-xs text-purple-700 mt-1 mb-2">OR</div>
                          <div className="flex flex-wrap gap-2 mt-1">
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_0.jpg</span> (primary)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_1.jpg</span> (2nd view)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_2.jpg</span> (3rd view)
                            <span className="font-mono bg-indigo-100 px-1 rounded">VPR63U_3.jpg</span> (4th view)
                          </div>
                        </li>
                        <li className="mb-1">Supported formats: .jpg, .jpeg, .png, .webp, .gif</li>
                        <li className="font-semibold text-indigo-800">Important: Place all images in the root of the zip file, not in subfolders</li>
                      </ul>
                    </p>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 border border-indigo-100">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-bold text-indigo-900">Import Batches</h2>
              <p className="text-indigo-600 mt-1">Manage product import batches</p>
            </div>
            <button 
          ...
          {loadingBatches ? (
            <div className="flex items-center justify-center h-64">
              <div className="flex flex-col items-center">
                ...
                <p className="mt-3 text-indigo-600 text-lg">Loading batches...</p>
              </div>
            </div>
          ) : (
            <div className="overflow-x-auto">
              {batches.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-lg text-indigo-800 mt-4">No import batches found.</p>
                  <p className="text-sm text-indigo-600 mt-2">Batches will appear here after importing products.</p>
                  <button 
                    onClick={() => setActiveTab('import')} 
                    className="mt-4 px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors duration-150"
                  >
                    Import Products
                  </button>
                </div>
                ...
                <>
                  <table className="min-w-full divide-y divide-indigo-200">
                    <thead className="bg-indigo-50">
                      <tr>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Batch ID</th>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Category</th>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Source File</th>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Product Count</th>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Import Date</th>
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-indigo-800 uppercase tracking-wider">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-indigo-200">
                      {batches.map((batch) => (
                        <tr key={batch._id} className="hover:bg-indigo-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-900">{batch._id}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-800">{batch.category}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-800">{batch.sourceFile}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-800">{batch.count}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-800">
                            {batch.date ? new Date(batch.date).toLocaleDateString() + ' ' + new Date(batch.date).toLocaleTimeString() : 'N/A'}
                          </td>
                  ...
                  <div className="mt-8 flex justify-center">
                    <button 
                      onClick={() => setActiveTab('import')}
                      className="flex items-center px-5 py-2 border border-indigo-500 text-indigo-600 hover:bg-indigo-50 rounded-lg shadow-sm transition-colors duration-150"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
